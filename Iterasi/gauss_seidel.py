def print_persamaan(A, b):
    ...
    for i in range(n):
        ...
        # Penempatan tanda '=' agar rapi di tengah
...
def gauss_seidel_iteration(A, b, tebakan_awal, epsilon, max_iter=100):
    ...
    awal_str = " | ".join([f"{val:10.2f}" for val in x])
    ...
    for iterasi in range(1, max_iter + 1):
        ...
        x_str = " | ".join([f"{val:10.2f}" for val in x])
    ...
    print("-" * (25 + 11 * n))
    print("Peringatan: Iterasi mencapai batas maksimal.")
    return x
...
# 3. Menjalankan fungsi Iterasi Gauss-Seidel
tebakan_awal = [0.0, 0.0, 0.0]
batas_error = 1e-5
hasil_x = gauss_seidel_iteration(A_urut, b_urut, tebakan_awal, batas_error)
